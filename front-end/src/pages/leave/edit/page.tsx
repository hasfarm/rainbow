...
import { leaveTypeLabels, leaveTypeIcons, leaveTypeColors, type LeaveType, type LeaveRecord, mockLeaves } from '@/mocks/leaves';
...
export default function LeaveEditPage() {
  ...
  // Pre-fill form from existing leave data
  useEffect(() => {
    if (!leave || initialized) return;
    if (leave.status !== 'pending') {
      navigate(`/leave/${id}`, { replace: true });
      return;
    }

    setSelectedType(leave.type);

    const isHourly = !!leave.startTime || !!leave.endTime;
    if (isHourly) {
      setLeaveMode('hourly');
      setSingleDate(leave.startDate);
    ...
    if (leave.handoverTo) {
      setNeedHandover(true);
      setHandoverTo(leave.handoverTo);
      setHandoverNote(leave.handoverNote || '');
    }

    setApprover(leave.approver);
    setReason(leave.reason);
    setInitialized(true);
  }, [leave, initialized, id, navigate]);
  ...
  // Redirect if leave not found or not pending
  if (!leave) {
    return (
  ...
  if (leave.status !== 'pending') {
    return (
      <div className="px-4 pt-6 pb-4">
        <div className="flex flex-col items-center justify-center py-20 text-center">
          <span className="w-16 h-16 bg-primary-50 rounded-2xl flex items-center justify-center mb-4">
            <i className="ri-lock-line text-2xl text-primary-400"></i>
          </span>
          <p className="text-sm font-medium text-foreground-600 mb-2">Không thể chỉnh sửa đơn này</p>
          <p className="text-xs text-foreground-400 mb-4">Chỉ có thể chỉnh sửa đơn đang ở trạng thái Chờ duyệt</p>
          <Link to={`/leave/${id}`} className="text-xs text-primary-500 font-medium hover:text-primary-600 transition-colors cursor-pointer">
            ← Quay lại chi tiết
          </Link>
        </div>
      </div>
  ...
  const validate = (): boolean => {
    ...
    if (leaveMode === 'full_day') {
      ...
      if (startDate && endDate && endDate < startDate) newErrors.endDate = 'Ngày kết thúc phải sau ngày bắt đầu';
    }
    if (leaveMode === 'hourly') {
      if (!singleDate) newErrors.singleDate = 'Vui lòng chọn ngày';
  ...
  const handleSubmit = async () => {
    ...
    // Update the leave record in mockLeaves
    ...
    if (idx !== -1) {
      if (leaveMode === 'full_day') {
        mockLeaves[idx] = {
          ...
          endTime: null,
          reason: reason.trim(),
          handoverTo: needHandover ? handoverTo : null,
  ...
  return (
    <div className="px-4 pt-6 pb-8">
      ...
      <div className="flex items-center gap-3 mb-6">
        <Link to={`/leave/${id}`} className="w-9 h-9 bg-background-100 rounded-lg flex items-center justify-center hover:bg-background-200 transition-colors cursor-pointer shrink-0">
          <i className="ri-arrow-left-line text-foreground-600"></i>
        </Link>
        <h1 className="text-lg font-heading font-bold text-foreground-950">Chỉnh sửa đơn nghỉ</h1>
        <span className="ml-auto inline-flex items-center px-2.5 py-1 text-[11px] font-medium rounded-full bg-secondary-100 text-secondary-700 whitespace-nowrap">
          Mã: {id}
        </span>
      </div>
